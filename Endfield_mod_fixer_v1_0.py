...
def apply_endfield_stage(content: str, newline: str) -> tuple[str, StageResult]:
    ...
    if END_FIELD_FIXED_MARKER in content:
        return content, StageResult("endfield_mod_fix", False, ["skipped: marker already present"])
    ...
    if details:
        content = content.rstrip() + newline + newline + END_FIELD_FIXED_MARKER + newline
...
def apply_ps_t_shift_stage(content: str, newline: str) -> tuple[str, StageResult]:
    ...
    if END_FIELD_FIXED_MARKER in content and END_FIELD_FIXED_MARKER not in new_content:
        new_content = new_content.rstrip() + newline + newline + END_FIELD_FIXED_MARKER + newline
        details.append("preserved endfield_mod_fix marker")
...
def process_text(
    ...
    if skip_new_version:
        is_new_version, reason = detect_new_version_mod(content, mapping)
        if is_new_version:
            guard_stage = StageResult("new_version_guard", False, [f"skipped: {reason}"])
            stages = pre_stages + [guard_stage]
            if END_FIELD_13_OLD_HASH in collect_hash_values(content):
                content, stage = apply_endfield_13_stage(content)
                guard_stage.details = [f"skipped previous stages: {reason}; ran 1.3 fixer only"]
                stages.append(stage)
            if enable_fixmenu:
